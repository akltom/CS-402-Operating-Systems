...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        /* Check the validation of address range in usr space */
        /*
        if((uint32_t)vaddr < USER_MEM_LOW && (uint32_t)vaddr >= USER_MEM_HIGH){
                dbg(DBG_ERROR, "bbbbbbbbbbbbbbbb\n");
                return 0;
        }
        */
        vmarea_t *vmarea = NULL;

        if((vmarea = vmmap_lookup(p->p_vmmap, ADDR_TO_PN(vaddr)))== NULL){
                dbg(DBG_PRINT,"(GRADING3D)\n");
                return 0;
        }
    /*
        if ((perm & PROT_READ) <= (vmarea->vma_prot & PROT_READ) &&
             (perm & PROT_WRITE) <= (vmarea->vma_prot & PROT_WRITE) &&
             (perm & PROT_EXEC) <= (vmarea->vma_prot & PROT_EXEC)){
...
int
 range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
 {
         uint32_t diff = 0;
         while (diff < len) {
                 if (addr_perm(p, (void *) ((uint32_t) avaddr + diff), perm) == 0) {
                         dbg(DBG_PRINT, "(GRADING3D)\n");
                         return 0;
                 }
