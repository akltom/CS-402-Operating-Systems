...
sys_read(read_args_t *arg)
{
        ...
        read_args_t kread_args;
        int kern_ret = copy_from_user(&kread_args, arg, sizeof(kread_args));
        /*not included in self_check*/
        /*
        if (kern_ret < 0) {
                dbg(DBG_ERROR,"aaaaaaaaaaaaaa\n");
                curthr->kt_errno = -kern_ret;
                return -1;
        }
        */
        void *buf = NULL;
        ...
        int read_ret = do_read(kread_args.fd, buf,kread_args.nbytes);

        if(read_ret < 0){
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                curthr->kt_errno = -read_ret;
                page_free(buf);
                return -1;
        }

        dbg(DBG_PRINT, "(GRADING3D 1)\n");
        int err = copy_to_user(arg->buf, buf, read_ret);
        /*not included in self-check*/
        /*
        if (err < 0) {
                dbg(DBG_ERROR, "ccccccccccccccc\n");
                curthr->kt_errno = -err;
                page_free(buf);
                return -1;
        }
...
sys_write(write_args_t *arg)
{
        ...
        write_args_t kwrite_args;
        int kern_ret = copy_from_user(&kwrite_args, arg, sizeof(kwrite_args));
        /*not included in self check*/
        /*
        if (kern_ret < 0) {
                dbg(DBG_ERROR, "dddddddddddd\n");
                curthr->kt_errno = -kern_ret;
                return -1;
        }
        */
        ...
        int err = copy_from_user(buf, kwrite_args.buf, kwrite_args.nbytes);
        /*not included in self check*/
        /*
        if ( err < 0) {
                dbg(DBG_ERROR, "eeeeeeeeeeeeeeeee\n");
                curthr->kt_errno = -err;
                page_free(buf);
                return -1;
        }
        */
        int write_ret = do_write(kwrite_args.fd, buf, kwrite_args.nbytes);
        if(write_ret < 0){
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                curthr->kt_errno = -write_ret;
                page_free(buf);
                return -1;
        }
...
sys_getdents(getdents_args_t *arg)
{
     /* NOT_YET_IMPLEMENTED("VM: sys_getdents"); */
        ...
        getdents_args_t kdents_args;
        int kdents_ret = copy_from_user(&kdents_args, arg, sizeof(getdents_args_t));
        /*not included in self check*/
        /*
        if (kdents_ret < 0){
                dbg(DBG_ERROR, "hhhhhhhhhh\n");
                curthr->kt_errno = -kdents_ret;
                return -1;
        }
        */
        dirent_t d;

        int ndirents = (int) kdents_args.count / sizeof(dirent_t);
        int ret = 0;
        int i;

        for (i = 0; i < ndirents; i++){
                int curr = do_getdent(kdents_args.fd, &d);

                if (curr < 0){
                        dbg(DBG_PRINT, "(GRADING3D)\n");
                        curthr->kt_errno = -curr;
                        return -1;
                }else if (curr == 0){
                ...
                dbg(DBG_PRINT, "(GRADING3B)\n");
                int err = copy_to_user(kdents_args.dirp + i, &d, curr);
                /*not included in self check*/
                /*
                if (err < 0) {
                        dbg(DBG_ERROR, "jjjjjjjjjjjjjjjjjjjjjj\n");
                        curthr->kt_errno = -err;
                        return -1;
                }
                */
