...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
        ...
        pframe_t *pft = pframe_get_resident(o, pagenum);

        if (!pft)
        {       
                dbg(DBG_PRINT, "(GRADING3B)\n");
                pft = pframe_alloc(o, pagenum);
                ...
                {       
                        ...
                        dbg(DBG_PRINT, "(GRADING3EB)\n");
                        return -ENOMEM;
                        /*}*/
                }
                dbg(DBG_PRINT, "(GRADING3B)\n");
                
                int ret = pframe_fill(pft);
                if(ret < 0){
                ...
                if (pframe_is_busy(pft))
                {       
                        dbg(DBG_PRINT, "(GRADING3B)\n");
                        sched_sleep_on(&pft->pf_waitq);
                } else {
                        dbg(DBG_PRINT, "(GRADING3B)\n");
                }
